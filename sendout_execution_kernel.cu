...
#if !defined(__SENDOUT_EXECUTION_KERNEL_CU__)
#define __SENDOUT_EXECUTION_KERNEL_CU__

#include "pcuda_types.h"
#include "p_system.h"

__global__ void sendout_execution_kernel(const Rodruleset rules, Membraneset membranes,
    ...
    if (type == RULE_SEND_OUT) {
        membranes[bid].charge = rules[rule * numLabs * NUMBER_OF_CHARGES + label * NUMBER_OF_CHARGES + charge].rinfo>>6 & 0x0003;
        atomicAdd(&(skin[rules[rule * numLabs * NUMBER_OF_CHARGES + label * NUMBER_OF_CHARGES + charge].rsodd[0]]), 1);
        //skin[rules[rule * numLabs * NUMBER_OF_CHARGES + label * NUMBER_OF_CHARGES + charge].rsodd[0]] +=  1;
        //membranes[bid].charge = rules[rule * numLabs * NUMBER_OF_CHARGES + label * NUMBER_OF_CHARGES + charge].rinfo>>6 & 0x0003;
    } 
}

#endif	/* __SENDOUT_EXECUTION_KERNEL_CU__ */
